  ...
  return (
    <div className="bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800/80 rounded-2xl p-5 space-y-4 shadow-sm">
      <div className="flex items-center space-x-2">
        ...
        <h2 className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">Output Settings</h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Container format toggle */}
        <div className="space-y-1.5">
          <label className="text-[11px] font-medium text-zinc-600 dark:text-zinc-400">Container Format</label>
        ...
        <div className="md:col-span-2 space-y-1.5">
          <label className="text-[11px] font-medium text-zinc-600 dark:text-zinc-400">Destination File</label>
          <div className="flex items-center space-x-2">
            <input
              type="text"
              value={outputPath}
              ...
              className="flex-1 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700/80 rounded-xl px-3 py-1.5 text-xs font-mono text-zinc-900 dark:text-zinc-200 focus:outline-none focus:border-blue-500 truncate"
              title={outputPath}
            />
            <button
              type="button"
              onClick={handleBrowseOutput}
              className="px-3 py-1.5 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 active:bg-zinc-300 dark:active:bg-zinc-600 text-zinc-700 dark:text-zinc-200 rounded-xl text-xs font-medium flex items-center space-x-1.5 transition-colors shrink-0"
            >
          ...
          <p className="text-[10px] text-zinc-500 dark:text-zinc-400 truncate">
            Target file: <span className="font-mono text-zinc-700 dark:text-zinc-300">{extractFilename(outputPath) || 'Not set'}</span>
          </p>
        </div>
      </div>
      ...
      {previewCommandString && (
        <div className="pt-2 border-t border-zinc-200 dark:border-zinc-800/60">
          <button
            ...
            className="w-full flex items-center justify-between text-xs text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-300 py-1"
          >
          ...
          {showPreview && (
            <div className="mt-2 bg-zinc-950 rounded-xl border border-zinc-800 p-3 relative group">
              <pre className="text-[11px] font-mono text-zinc-200 overflow-x-auto whitespace-pre-wrap break-all pr-12 leading-relaxed">
                {previewCommandString}
              </pre>
              <button
                type="button"
