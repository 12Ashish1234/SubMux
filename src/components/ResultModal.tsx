...
export const ResultModal: React.FC<ResultModalProps> = ({ result, error, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 dark:bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700/80 rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150 text-zinc-900 dark:text-zinc-100">
        {/* Success View */}
        {result && (
          <>
            ...
            <div className="bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 p-4 space-y-2">
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-500 dark:text-zinc-400 font-medium">Output File</span>
                <span className="text-zinc-800 dark:text-zinc-200 font-semibold">
                  {extractFilename(result.output_path)}
                </span>
              </div>
              <p className="text-xs text-zinc-600 dark:text-zinc-400 font-mono break-all bg-white dark:bg-zinc-900/60 p-2 rounded-lg border border-zinc-200 dark:border-zinc-800">
                {result.output_path}
              </p>
              {result.output_size_bytes > 0 && (
                <div className="flex justify-between items-center text-xs pt-1 border-t border-zinc-200 dark:border-zinc-800/80">
            ...
            <div className="flex items-center justify-end space-x-3 pt-2">
              ...
              <button
                onClick={onClose}
                className="px-4 py-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 active:bg-zinc-300 dark:active:bg-zinc-600 text-zinc-700 dark:text-zinc-200 rounded-xl text-xs font-semibold transition-colors"
              >
                Done
              </button>
            </div>
          ...
          <>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400">FFmpeg Error Output</span>
                <button
                  onClick={handleCopyError}
                  className="px-2 py-1 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded text-xs flex items-center space-x-1 transition-colors"
                >
                  {copied ? (
                    <>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <button
                onClick={onClose}
                className="px-4 py-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-200 rounded-xl text-xs font-semibold transition-colors"
              >
                Dismiss
              </button>
            </div>
