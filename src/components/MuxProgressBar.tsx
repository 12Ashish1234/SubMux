...
export const MuxProgressBar: React.FC<MuxProgressBarProps> = ({ progress, isMuxing, onCancel }) => {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900/90 border border-blue-500/30 dark:border-blue-500/40 rounded-2xl p-5 space-y-3.5 shadow-xl shadow-blue-500/5 animate-in fade-in duration-200">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          <button
            type="button"
            onClick={onCancel}
            className="px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 active:bg-rose-500/30 text-rose-600 dark:text-rose-300 border border-rose-500/20 dark:border-rose-500/30 rounded-xl text-xs font-semibold flex items-center space-x-1.5 transition-colors shadow-sm"
          >
            <Square className="w-3 h-3 fill-current" />
            <span>Stop Muxing</span>
          </button>
          <span className="text-2xl font-bold font-mono text-blue-600 dark:text-blue-400 min-w-[60px] text-right">
            {percentage}%
          </span>
        </div>
      </div>
      ...
      <div className="w-full bg-zinc-100 dark:bg-zinc-950 rounded-full h-3 overflow-hidden p-0.5 border border-zinc-200 dark:border-zinc-800">
        <div
      ...
      {/* Detailed Metrics */}
      <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400 font-mono pt-1">
